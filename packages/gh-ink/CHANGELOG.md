...
- 71c7833: A standing status strip, a stack of rails, and `c` on a rail row.
...
- c02824c: ↵ on a rail row opens the initiative in the drill; the rail carries a mark.
...
- 76199aa: A blank line under each comment's author rule.

  The body sat flush against `── author ──`, so one that opens with a heading or a bold line — most PR descriptions do — read as part of the rule. Same spacing over and under it now, in the conversation and in review threads alike.
...
- 8fc48e3: The rail is there on a launch painted from the cache, and takes a third of a wide frame.
...
- da4c77c: An unreviewed PR on a repo you own is yours, and the login the bands read against is no longer optional.
...
- Updated dependencies [da4c77c]
  - @kud/gh-workflow@0.11.0
...
- Updated dependencies [1897583]
  - @kud/gh-workflow@0.10.0

## 0.51.2

### Patch Changes

- 854b8ac: A thread you opened on somebody else's PR is theirs until they answer.

  `threads` fires on any unresolved thread, PR-wide and author-blind, and from a PR you had already reviewed the `YOURS` table claimed it unconditionally — so a review comment of your own that nobody had replied to sat under Your move while the turn arrow on the same row read `→`, you spoke last. The claim is now a conjunction: from `spoken`, `threads` is yours only while somebody else has the last word, an arm beside the `authored` one it mirrors. When the login is unknown it reads as theirs, the file's own bias for a search that is provably not waiting on you. From `queued` nothing changes — the review is the claim there, and `computeHealth` ranks `threads` above `waiting`, so a re-requested review with your own unanswered thread stays yours.

  On the way: `filterByOrigin`, `filterBySearch` and `filterByRepos` re-lay the kept rows out and took no `login`, so every band computed after typing a search or toggling a filter threw `lastActor` away — a row claimed by somebody's last word fell from Your move to Their move the moment `/` was pressed. They take an optional `login` now and the inbox passes it.

- Updated dependencies [854b8ac]
  - @kud/gh-workflow@0.9.0

## 0.51.1

### Patch Changes

- 17c1553: The diff size shows on every PR row, your own included.

  It was gated on "somebody else's PR" for one morning, on the argument that you know the size of your own. You do not, not across seven of them — the number is how a list of your own PRs is triaged too — and a cell that appears on the row above and not on yours reads as a column that failed to fill rather than as a decision.

## 0.51.0

### Minor Changes

- 65628c9: An extension can now declare which row kinds it is the in-tree view for, and ↵ on such a row mounts it the way ↵ on a PR mounts `PrView`.
...
- 5bab71b: The diff size is coloured, additions green and deletions red, on the inbox row and the PR header alike.
...
- Updated dependencies [29e5587]
- Updated dependencies [1170b7d]
  - @kud/gh-workflow@0.6.0
...
- Updated dependencies [c20b3b9]
  - @kud/gh-workflow@0.5.0
...
- Updated dependencies [66f058d]
  - @kud/gh@0.14.0
  - @kud/gh-workflow@0.4.1

## 0.46.0

### Minor Changes

- b490782: The rail is twelve columns wider, and the list recedes while the rail holds the arrows.

  Two complaints about the same panel. At 40 columns the label line had 33 to spend, which is under half a real initiative title — the rail listed keys with an ellipsis after them, and a key you cannot read a title against is a key you have to open something else to place. `SIDEBAR_COLS` is 52.

  The second is that focus was invisible. The keymap has always shut the list out the moment `⇥` hands the rail the arrows, so every row binding below that branch was already inert — but both cursors stayed lit, and nothing on screen said which one `↵` would act on. The list now takes the same backdrop an overlay gives it: one vocabulary for "the region you are not standing in", not two. The row you will come back to keeps its `❯`, in the backdrop's tone.

  The rail's width is the lever the row-fitting specs pull, so their rungs moved with it — `narrow.test.tsx` and `labels.test.tsx` are recalibrated, and the light-pressure fixture is depth 0 so the ladder is measured in the columns the spec means rather than in a tree prefix.
...
- 8c0c4f2: The cursor follows the row it was on across a refresh, and stops being kicked off repo headers.
...
- Updated dependencies [9dd17b8]
  - @kud/gh@0.12.1
  - @kud/gh-workflow@0.3.1

## 0.45.4

### Patch Changes

- d58b360: Row transit ramps run at the ticker's own 150ms, the same tempo as the tab pulse and the merge sparkle.
...
- c81de33: The tab badge counts the work again, at whatever depth the board files it. `topLevelCount` is now `workCount`.
...
- 142abc6: The on-disk glance is invalidated once, so upgrading does not open with the burst it just fixed.
