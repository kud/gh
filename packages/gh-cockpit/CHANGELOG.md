...
- 689c2b8: The PR header's summary line gets a hierarchy: bold orange size, plain file count, dim provenance.
...
- 4702393: Opening a failing check's log works again when the log carries colour, which is most of them.
...
- a8ce59a: The delegated agent is seeded with plain prose carrying the URL, so every agent can be offered again.
...
- effa9ac: The AI launcher offers only agents that understand the prompt it hands them, which today means Claude Code alone.
