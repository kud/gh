...
- aa5cded: `issueCount` no longer decides whether a source was truncated, because it is not a total.
...
- 66f058d: A third tier that gives the overflow rows a verdict, and a correction to why the first two exist.
...
- 2d3564b: `InboxQueryOptions.limits` lets a caller override any source's `first`, so the two-tier fetch a truncated source needs is expressible from outside this file.
